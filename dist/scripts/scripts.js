...
"A file will be created in this directory for each key from the config map or secret. The file contents will be the value of the key.": "在该目录下将给每个来自config map 或者secret的key创建一个文件 。 文件内容将是密钥的值。",
...
"A liveness probe checks if the container is still running. If the liveness probe fails, the container is killed.": "Liveness Probe检查容器是否仍在运行。 如果Liveness Probe失败，容器被杀死。",
...
"An image stream or template is required.": "image stream 或 模板是必选项.",
...
"Automatically start a new deployment when the deployment configuration changes": "deployment configuration更改时自动启动新deployment",
"Automatically start a new deployment when the image changes": "镜像更改时自动启动新deployment",
...
"Builds triggered from this Build Configuration will run one at the time. When a currently running build completes, the next build that will run is the latest build created. Other queued builds will be cancelled.": "从此Build Configuration触发的Builds将在当时运行一个。 当前正在运行的build完成时，将运行的下一个build是创建的最新build。 其他排队的build将被取消。",
...
Capacity: "容量",
"Capacity:": "容量：",
"Catalog category {{category}} not found.": "找不到Catalog category{{category}}。",
"Catalog category {{category}}/{{subcategory}}not found.": "未找到Catalog category{{category}} / {{subcategory}}。",
"Catalog Home (Default)": "首页（默认）",
Certificate: "证书",
"Certificate:": "证书：",
Certificates: "证书",
change: "改变",
...
"Continue to the project 概况": "继续至项目概述",
